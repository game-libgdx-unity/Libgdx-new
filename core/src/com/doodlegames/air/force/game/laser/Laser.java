...
import com.doodlegames.air.force.enemy.level5.MyVector2;
...
public abstract class Laser extends DynamicGameObject {
   ...
   public void setDeltaY(float var1) {
      this.deltaY = var1;
      MyVector2.tmp.set(0.0F, var1).rotate(this.shooter.getLaserAngle()).add(this.shooter.getShootingPoint());
      this.setPosition(MyVector2.tmp.x, MyVector2.tmp.y);
   ...
   public void update(float var1) {
      if(!this.shooter.laseSleep()) {
         ...
         if(this.angle != var3) {
            MyVector2.tmp.set(0.0F, this.deltaY).rotate(var3).add(var4);
            this.setPosition(MyVector2.tmp.x, MyVector2.tmp.y);
            this.angle = var3;
         }
